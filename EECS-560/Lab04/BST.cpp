...
void BST::LeftSideView() {
  int max_level = 0;
  LeftSideView(m_root, 1, &max_level);
}

void BST::LeftSideView(Node *root, int level, int *max_level) {
  // Base Case
  if (root == NULL)
    return;
  ...
  // then left subtree
  LeftSideView(root->GetLeft(), level + 1, max_level);
  LeftSideView(root->GetRight(), level + 1, max_level);
}

void BST::RightSideView() {
  int max_level = 0;
  RightSideView(m_root, 1, &max_level);
}
